...
config.sftpanel = ConfigSubsection()
config.sftpanel.cronmanager_commandtype = NoSave(ConfigSelection(choices = [ ('custom',_("Custom")),('predefined',_("Predefined")) ]))
config.sftpanel.cronmanager_cmdtime = NoSave(ConfigClock(default=0))
config.sftpanel.cronmanager_cmdtime.value, mytmpt = ([0, 0], [0, 0])
config.sftpanel.cronmanager_user_command = NoSave(ConfigText(fixed_size=False))
config.sftpanel.cronmanager_runwhen = NoSave(ConfigSelection(default='Daily', choices = [('Hourly', _("Hourly")),('Daily', _("Daily")),('Weekly', _("Weekly")),('Monthly', _("Monthly"))]))
config.sftpanel.cronmanager_dayofweek = NoSave(ConfigSelection(default='Monday', choices = [('Monday', _("Monday")),('Tuesday', _("Tuesday")),('Wednesday', _("Wednesday")),('Thursday', _("Thursday")),('Friday', _("Friday")),('Saturday', _("Saturday")),('Sunday', _("Sunday"))]))
config.sftpanel.cronmanager_dayofmonth = NoSave(ConfigInteger(default=1, limits=(1, 31)))
...
class SetupCronConf(Screen, ConfigListScreen):
	...
	def createSetup(self):
		...
		if f:
			...
			predefinedlist.sort()
		config.sftpanel.cronmanager_predefined_command = NoSave(ConfigSelection(choices = predefinedlist))
		self.editListEntry = None
		self.list = []
		self.list.append(getConfigListEntry(_("Run how often ?"), config.sftpanel.cronmanager_runwhen))
		if config.sftpanel.cronmanager_runwhen.value != 'Hourly':
			self.list.append(getConfigListEntry(_("Time to execute command or script"), config.sftpanel.cronmanager_cmdtime))
		if config.sftpanel.cronmanager_runwhen.value == 'Weekly':
			self.list.append(getConfigListEntry(_("What Day of week ?"), config.sftpanel.cronmanager_dayofweek))
		if config.sftpanel.cronmanager_runwhen.value == 'Monthly':
			self.list.append(getConfigListEntry(_("What Day of month ?"), config.sftpanel.cronmanager_dayofmonth))
		self.list.append(getConfigListEntry(_("Command type"), config.sftpanel.cronmanager_commandtype))
		if config.sftpanel.cronmanager_commandtype.value == 'custom':
			self.list.append(getConfigListEntry(_("Command To Run"), config.sftpanel.cronmanager_user_command))
		else:
			self.list.append(getConfigListEntry(_("Command To Run"), config.sftpanel.cronmanager_predefined_command))
	...
	def checkentry(self):
		msg = ''
		if (config.sftpanel.cronmanager_commandtype.value == 'predefined' and config.sftpanel.cronmanager_predefined_command.value == '') or config.sftpanel.cronmanager_commandtype.value == 'custom' and config.sftpanel.cronmanager_user_command.value == '':
	...
	def saveMycron(self):
		hour = '%02d' % config.sftpanel.cronmanager_cmdtime.value[0]
		minutes = '%02d' % config.sftpanel.cronmanager_cmdtime.value[1]
		if config.sftpanel.cronmanager_commandtype.value == 'predefined' and config.sftpanel.cronmanager_predefined_command.value != '':
			command = config.sftpanel.cronmanager_predefined_command.value
		else:
			command = config.sftpanel.cronmanager_user_command.value

		if config.sftpanel.cronmanager_runwhen.value == 'Hourly':
			newcron = minutes + ' ' + ' * * * * ' + command.strip() + '\n'
		elif config.sftpanel.cronmanager_runwhen.value == 'Daily':
			newcron = minutes + ' ' + hour + ' * * * ' + command.strip() + '\n'
		elif config.sftpanel.cronmanager_runwhen.value == 'Weekly':
			if config.sftpanel.cronmanager_dayofweek.value == 'Sunday':
				newcron = minutes + ' ' + hour + ' * * 0 ' + command.strip() + '\n'
			elif config.sftpanel.cronmanager_dayofweek.value == 'Monday':
				newcron = minutes + ' ' + hour + ' * * 1 ' + command.strip() + '\n'
			elif config.sftpanel.cronmanager_dayofweek.value == 'Tuesday':
				newcron = minutes + ' ' + hour + ' * * 2 ' + command.strip() + '\n'
			elif config.sftpanel.cronmanager_dayofweek.value == 'Wednesday':
				newcron = minutes + ' ' + hour + ' * * 3 ' + command.strip() + '\n'
			elif config.sftpanel.cronmanager_dayofweek.value == 'Thursday':
				newcron = minutes + ' ' + hour + ' * * 4 ' + command.strip() + '\n'
			elif config.sftpanel.cronmanager_dayofweek.value == 'Friday':
				newcron = minutes + ' ' + hour + ' * * 5 ' + command.strip() + '\n'
			elif config.sftpanel.cronmanager_dayofweek.value == 'Saturday':
				newcron = minutes + ' ' + hour + ' * * 6 ' + command.strip() + '\n'
		elif config.sftpanel.cronmanager_runwhen.value == 'Monthly':
			newcron = minutes + ' ' + hour + ' ' + str(config.sftpanel.cronmanager_dayofmonth.value) + ' * * ' + command.strip() + '\n'
		else:
			command = config.sftpanel.cronmanager_user_command.value
		...
		out.close()
		rc = system('crontab /etc/cron/crontabs/root -c /etc/cron/crontabs')
		config.sftpanel.cronmanager_predefined_command.value = 'None'
		config.sftpanel.cronmanager_user_command.value = 'None'
		config.sftpanel.cronmanager_runwhen.value = 'Daily'
		config.sftpanel.cronmanager_dayofweek.value = 'Monday'
		config.sftpanel.cronmanager_dayofmonth.value = "1"
		config.sftpanel.cronmanager_cmdtime.value, mytmpt = ([0, 0], [0, 0])
		self.close()
